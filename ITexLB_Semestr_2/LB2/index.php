...
<body>
    ...
    <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['league_name'])) {
            ...
            if (empty($teamsStats)) {
                ...
                echo "<p style='font-size: 20px'>Таблиця для ліги: " . htmlspecialchars($leagueName) . "</p>";
                ...
                foreach ($teamsStats as $teamName => $stats) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($stats['NameOfTeam']) . "</td>";
                ...
                echo "<script>saveToHistory('league_table', '" . addslashes($leagueName) . "', " . json_encode($result) . ");</script>";
            }
        }
    ?>
    ...
    <?php
       ...
       if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['players_list'])) {
           $TeamName = trim($_POST['players_list']);
   
           $SelectedTeam = $teamsCollection->findOne(['name' => $TeamName]);
   
           if ($SelectedTeam) {
               echo "<p style='font-size: 20px'>Список гравців команди " . htmlspecialchars($SelectedTeam['name']) . ":</p>";
               ...
               echo "<script>saveToHistory('players_list', '" . addslashes($TeamName) . "', " . json_encode($result) . ");</script>";
           } else {
    ...
    <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['team_name'])) {
            $TeamName2 = trim($_POST['team_name']);  
            ...
            if ($games->isDead()) {        
                ...
                echo "<p style='font-size: 20px'>Список ігор для команди " . htmlspecialchars($TeamName2) . ":</p>";
                echo "<ul style='font-size: 20px'>";

                $result = "Список ігор для команди " . $TeamName2 . ":\n";
                foreach ($games as $game) {
                    echo "<li>Ліга: " . htmlspecialchars($game['League']) . "<br>";
                    echo "Дата: " . htmlspecialchars(date("Y-m-d H:i:s", $game['Data'])) . "<br>";
                    echo "Місце проведення: " . htmlspecialchars($game['Place']) . "<br><br>";
                ...
                echo "<script>saveToHistory('games_list', '" . addslashes($TeamName2) . "', " . json_encode($result) . ");</script>";
            }
        }
    ?>
